...
const LARGE_LANE_WIDTH: f32 = 3.7;
...
pub fn circle_curve(pos1: Vector3<f32>, dir1: Vector3<f32>, pos2: Vector3<f32>) -> Vec<Vector3<f32>> {
    let diff = pos2 - pos1;
...
pub fn circle_curve_fudged(pos1: Vector3<f32>, dir1: Vector3<f32>, pos2: Vector3<f32>, dir2: Vector3<f32>) -> Vec<Vector3<f32>>  {
    let diff = pos2 - pos1;
    let r = dir1 * circle_scale(diff, dir1);
...
pub fn double_snap_curve(pos1: Vector3<f32>, dir1: Vector3<f32>, pos2: Vector3<f32>, dir2: Vector3<f32>, lane_count: i32) -> anyhow::Result<Vec<Vec<Vector3<f32>>>> {
    ...
    if ndot(mirror(dir1, diff), dir2) > PRETTY_CLOSE &&
      ...
      diff.dot(dir1) >= PRETTY_CLOSE - 1.0 {
        ...
        if is_curve_too_small(dir1, center - pos1, lane_count) || is_curve_too_small(dir2, center - pos2, lane_count) {
...
fn simple_curve_points(pos1: Vector3<f32>, dir1: Vector3<f32>, pos2: Vector3<f32>, dir2: Vector3<f32>) -> anyhow::Result<Vec<Vector3<f32>>> {
...
fn s_curve_segment_length(v1: Vector3<f32>, r1: Vector3<f32>, v2: Vector3<f32>, r2: Vector3<f32>) -> f32 {
    let v = v2 - v1;
    let r = r2 - r1;
...
fn min_road_length(d1: Vector3<f32>, d2: Vector3<f32>, lane_count: i32) -> f32 {
    MIN_SEGMENT_LENGTH.max(LARGE_LANE_WIDTH * lane_count as f32 * 3.0 * d1.cross(d2).magnitude() / d1.magnitude() / d2.magnitude())
} 
...
fn is_eliptical(pos1: Vector3<f32>, dir1: Vector3<f32>, pos2: Vector3<f32>, dir2: Vector3<f32>) -> bool {
    let delta_pos = pos2 - pos1;
    if dir1.dot(dir2) > 0.0 {
        return false
    }
    if ndot(-delta_pos, dir2) < PRETTY_CLOSE - 1.0 ||
       ndot(delta_pos, dir1) < PRETTY_CLOSE - 1.0 {
        return false
    }
