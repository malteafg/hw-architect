...
impl Camera {
    ...
    pub fn calc_pos(&self) -> Vec3 {
        ...
        self.target
            + (Vec3::new(-cos_yaw, 0.0, -sin_yaw) * cos_pitch
                + Vec3::new(0.0, sin_pitch, 0.0))
                * self.dist_to_target
    }
}
...
impl CameraController {
    ...
    pub fn smooth_move(f: f32) -> f32 {
        (CAMERA_MOVE_SMOOTH_FACTOR + 1.0) / 2.0 * (2.0 * f - 1.0)
            / f32::sqrt(
                CAMERA_MOVE_SMOOTH_FACTOR.powi(2)
                    * ((2.0 * f - 1.0).powi(2) - 1.0)
    ...
    pub fn process_mouse(&mut self, event: input::MouseEvent) {
        match event {
            input::MouseEvent::Dragged(button, delta)
                if button == input::Mouse::Middle =>
            {
                self.delta_yaw +=
                    -MOUSE_HORIZONTAL_SENSITIVITY * delta.dx as f32;
                self.delta_pitch +=
                    MOUSE_VERTICAL_SENSITIVITY * delta.dy as f32;
                self.stop_move_progression();
            }
    ...
    fn update_progress(&mut self, dt: f32) {
        ...
        self.progress =
            f32::min(self.progress + self.progression_speed * dt, 1.0);
        let new_progress = (self.progression_function)(self.progress);

        camera.pitch = interpolate(
            old_progress,
            new_progress,
            camera.pitch,
            self.next_pitch,
        ...
        camera.yaw =
            interpolate(old_progress, new_progress, camera.yaw, self.next_yaw);
        ...
        camera.target = interpolate_vector(
            old_progress,
            new_progress,
            camera.target,
            self.next_target,
    ...
    fn update_manually(&mut self, dt: f32) {
        ...
        if self.velocity[0] > 0 {
            camera.target += calc_direction_vector(camera.yaw + PI)
                * speed
                * (self.velocity[0] as f32)
        }
        if self.velocity[1] > 0 {
            camera.target += calc_direction_vector(camera.yaw)
                * speed
                * (self.velocity[1] as f32)
        }
        if self.velocity[2] > 0 {
            camera.target += calc_direction_vector(camera.yaw + FRAC_PI_2)
                * speed
        ...
        if self.velocity[4] > 0 {
            camera.yaw = center(
                camera.yaw
                    + (self.velocity[4] as f32) * 5.0 * CAMERA_MOVE_SPEED * dt,
                PI,
            );
        }
        if self.velocity[5] > 0 {
            camera.yaw = center(
                camera.yaw
                    - (self.velocity[5] as f32) * 5.0 * CAMERA_MOVE_SPEED * dt,
                PI,
            );
        }

        if self.velocity[6] != 0 || self.velocity[7] != 0 {
            self.velocity[6] += if self.velocity[7] > 0
                || (self.velocity[7] == 0 && self.velocity[6] < 0)
            {
                1
            } else {
                -1
            ...
            let dist =
                new_dist(camera.dist_to_target, conseq_sum(self.velocity[6]));

            if self.velocity[6].abs() as f32
                >= (self.velocity[7].abs() as f32).sqrt()
